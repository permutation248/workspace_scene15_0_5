...
parser.add_argument('--alpha', default=0.1, type=float, help='Weight for Robust Contrastive Loss (if used)')
...
def ind2vec(ind, N=None, C=None):
    ...
    # 利用 numpy 的高级索引赋值
    # np.arange(N) 生成行索引，ind.flatten() 生成列索引
...
def generate_weighted_pseudo_labels(model, data_loader, quality_v1, quality_v2, num_classes, device):
    ...
    # --- 生成每个视图的初步标签 ---
    # 这里也可以像 snippet 一样用 init=centers 再跑一次 kmeans，
    # 或者直接 predict (因为 centers 已经很好了)
    # 为了简单且符合逻辑，直接 predict
    labels_v1 = kmeans.predict(features_v1)
    labels_v2 = kmeans.predict(features_v2)
...
def train_one_epoch(train_loader, model, criterions, optimizer, epoch, args, 
    ...
    for batch_idx, (x0, x1, _, _, _, _, indices) in enumerate(train_loader):
        ...
        if use_pseudo:
            # 1. 获取当前 batch 的伪标签 (One-Hot)
            # pseudo_labels 是全局张量，包含每个样本的类别索引
            batch_y = pseudo_labels[indices] # (B, )
            
            # 转换为 One-Hot (B, C)
            # 注意: 如果 ind2vec 是 numpy 函数，这里最好用 torch 自带的
            ...
            # 3. Softmax
            # 注意: 根据您的 snippet 使用 dim=1 进行标准的分类 Softmax
            pre0 = F.softmax(zp0, dim=1)
            pre1 = F.softmax(zp1, dim=1)
            
            # 4. 计算一致性权重 w (参照 snippet)
            # snippet: sim = pre0.mm(pre1.t()) -> (B, B)
            # diag = sim.diag() -> (B, ) 每个样本的一致性
            sim = pre0.mm(pre1.t())
            diag = sim.diag()
            # 归一化权重，使其和为 1 (或者保持 scale)
            # snippet 中: w = diag / diag.sum().detach()
            # 加上 detach 避免梯度回传到权重计算
            w_consistency = diag / (diag.sum().detach() + 1e-8)
            
            # 5. 计算 Cross Entropy 风格的损失
            # snippet: pp0 = p0.mm(pre0.t().log()).sum(1) 
            # 这在数学上等价于: sum(p_target * log(pre0), dim=1) 
            # 即提取目标类的 log probability
            ...
            # 6. 加权求和
            # 结合 snippet 的 w_consistency 和 我们自己的质量权重 w1/w2
            # 您的需求：质量低的样本权重低。w1/w2 已经是质量权重。
            # snippet 的 w_consistency 是视图间一致性权重。
            # 我们可以结合两者：weight = w_consistency * quality
            
            # 注意 pp0 是负数 (log prob)，loss 需要取负
            # snippet: lossa = - (args.beta * w * pp0).mean()
            
            # 这里的 batch_weight 结合了 "Batch内的一致性分布" 和 "样本自身的绝对质量"
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        # --- 评估 (修改为每个 epoch 或者是特定的 interval) ---
